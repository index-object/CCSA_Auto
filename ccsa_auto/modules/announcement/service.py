...
class AnnouncementService:
    ...
    def get_unread_announcements(user_id):
        ...
        try:
            # 获取用户已读公告ID
            read_announcement_ids = db.query(AnnouncementRead.announcement_id).filter_by(user_id=user_id).all()
            read_announcement_ids = [item[0] for item in read_announcement_ids]
            
            # 获取未读公告
            unread_announcements = db.query(Announcement).filter(~Announcement.id.in_(read_announcement_ids)).all()
    ...
    def mark_as_read(announcement_id, user_id):
        ...
        try:
            # 检查是否已经标记过
            existing_read = db.query(AnnouncementRead).filter_by(
                user_id=user_id,
    ...
    def get_announcement_stats():
        ...
        try:
            ...
            for ann in announcements:
                read_count = db.query(AnnouncementRead).filter_by(announcement_id=ann.id).count()
                ...
                stats.append({
                    ...
                    'read_rate': f"{(read_count / total_users * 100):.2f}%" if total_users > 0 else "0.00%"
                })
