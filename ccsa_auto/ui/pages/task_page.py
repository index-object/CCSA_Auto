...
def create_task_page():
    ...
    with ui.card().classes("w-full h-auto p-6 page-card task-page"):
            ...
            try:
                ...
                for task in tasks:
                    formatted_task = {
                        ...
                        "is_active": "是" if task.is_active else "否",
                        "execution_status": task.execution_status,
                        "external_status": task.external_status,
                        "next_run_time": format_datetime_for_display(
                            task.next_run_time
                        ),
                    ...
                    formatted_task["actions"] = [
                        ...
                        },
                        {
                            "label": "激活/停用",
                            "color": "warning",
                            "handler": lambda task_id=task.id,
                            is_active=task.is_active: toggle_task_active(
                                task_id, is_active
                            ),
                        },
                    ]
        ...
        with ui.row().classes("w-full mb-4 gap-2"):
            ui.button("刷新任务", on_click=refresh_tasks, icon="refresh").classes(
                "bg-blue-500 hover:bg-blue-600 text-white"
            )
            ui.button(
                "创建默认任务", on_click=create_default_tasks, icon="add"
                ...
                def update_stats():
                    ...
                    try:
                        ...
                        stats_label.set_text(
                            f"总计: {total} | 激活: {active} | 完成: {completed} | 失败: {failed}"
                        )
                    except Exception as e:
