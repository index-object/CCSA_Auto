...
def create_settings_page():
    ...
    with ui.card().classes("p-6 mb-6"):
        ui.label("任务调度配置").classes("text-xl font-semibold mb-4")

        with ui.row().classes("items-center gap-4 w-full mb-4"):
            ui.label("每日任务执行时间范围:").classes("w-40")
            ui.label(f"{daily_hours[0]}:00 - {daily_hours[1]}:00").classes(
                "text-gray-600"
            )

        with ui.row().classes("items-center gap-4 w-full mb-4"):
            ui.label("每周任务执行时间范围:").classes("w-40")
            ui.label(f"每周二 {weekly_hours[0]}:00 - {weekly_hours[1]}:00").classes(
                "text-gray-600"
            )

        with ui.row().classes("items-center gap-4 w-full mb-4"):
            ui.label("每月任务执行时间范围:").classes("w-40")
            ui.label(f"每月15日 {monthly_hours[0]}:00 - {monthly_hours[1]}:00").classes(
                "text-gray-600"
            )
    ...
    # Task Fixer Settings
    with ui.card().classes("p-6 mb-6"):
        ui.label("任务修复器").classes("text-xl font-semibold mb-4")

        with ui.row().classes("items-center gap-4 w-full"):
            ui.label("自动修复过期任务:").classes("w-40")
            ui.switch(
    ...
    # Log Settings
    with ui.card().classes("p-6 mb-6"):
        ui.label("日志配置").classes("text-xl font-semibold mb-4")

        with ui.row().classes("items-center gap-4 w-full mb-4"):
            ui.label("日志级别:").classes("w-40")
            ui.select(
        ...
        ui.label("日志文件存储在 logs 目录下").classes("text-sm text-gray-400")

    # Session Settings
    with ui.card().classes("p-6 mb-6"):
        ui.label("会话配置").classes("text-xl font-semibold mb-4")

        with ui.row().classes("items-center gap-4 w-full mb-4"):
            ui.label("无活动过期时间:").classes("w-40")
            with ui.row().classes("items-center gap-2"):
                ui.input(
    ...
    with ui.card().classes("p-6"):
        ui.label("系统信息").classes("text-xl font-semibold mb-4")

        with ui.row().classes("items-center gap-4 w-full mb-2"):
            ui.label("系统版本:").classes("w-40 text-gray-500")
            ui.label("CCSA Auto v2.0").classes("text-gray-700")

        with ui.row().classes("items-center gap-4 w-full mb-2"):
            ui.label("数据库:").classes("w-40 text-gray-500")
            ui.label("SQLite").classes("text-gray-700")

        with ui.row().classes("items-center gap-4 w-full mb-2"):
            ui.label("时区:").classes("w-40 text-gray-500")
            ui.label("Asia/Shanghai (UTC+8)").classes("text-gray-700")

        with ui.row().classes("items-center gap-4 w-full"):
            ui.label("框架:").classes("w-40 text-gray-500")
            ui.label("NiceGUI + SQLAlchemy").classes("text-gray-700")
